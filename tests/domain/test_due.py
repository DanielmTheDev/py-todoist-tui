...
def test_to_api_timed_emits_floating_datetime_under_date_key() -> None:
    # Todoist's Sync `due` object carries a timed due as an ISO datetime in the
    # `date` field (no `datetime` key exists); a `datetime` key is silently
    # ignored, clearing the due. No tz suffix keeps it a floating local time.
    due = Due(date=datetime.date(2026, 7, 21), time=datetime.time(9, 30))

    assert due.to_api == {"date": "2026-07-21T09:30:00"}
